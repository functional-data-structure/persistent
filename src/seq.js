export function seq(empty, size) {
	...
	Seq.prototype.slice = function (start = 0, end = this.len()) {
		...
		const [, rest] = this.tree.split((m) => m > start);
		const [slice] = rest.split((m) => m > end - start);
	...
	Seq.prototype.splice = function (start, deleteCount, ...items) {
		...
		if (deleteCount === 1) {
			if (items.length === 0) {
				return [
					new Seq(split.left.concat(split.right)),
					new Seq(empty(size).push(split.middle)),
				];
			}

			return [
				new Seq(split.left.append(items).concat(split.right)),
				new Seq(empty(size).push(split.middle)),
			];
		}
